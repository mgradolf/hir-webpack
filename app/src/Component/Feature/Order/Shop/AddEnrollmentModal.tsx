...
export const AddEnrollmentModal = (props: {
  buyer: IBuyer
  ...
  switch (props.sectionAddType) {
    case "buy":
      buttonLabel = "Buy Seats (Program Enrollment)"
      selectButton = (
        <Row justify="center">
          <Col span={12}>
            <Input.Search
              ...
              onSearch={(seatCount: string) => {
                if (seatCount && !isNaN(Number(seatCount))) {
                  ...
                  for (let i = 0; i < Number(seatCount); i++) {
                    requests.push(props.cartModelFunctionality.createProgramEnrollmentRequest(selectedItem.ProgramID))
                  }
    ...
    case "me":
        ...
        <Button
          ...
          onClick={() => {
            ...
            props.cartModelFunctionality
              .createProgramEnrollmentRequest(selectedItem.ProgramID, props.buyer.PersonID)
              .then((response) => {
                closeModal()
              })
          }}
      ...
      break
    case "others":
      buttonLabel = "Register Students (Program Enrollment)"
      selectButton = (
        <ContactListModal
          disabled={!selectedItem}
          onSelect={(selectedStudentIds) => {
            if (selectedStudentIds && selectedStudentIds.length > 0) {
              ...
              for (const id of selectedStudentIds) {
                requests.push(props.cartModelFunctionality.createProgramEnrollmentRequest(selectedItem.ProgramID, id))
              }
              Promise.all(requests).then((responses) => {
                closeModal()
              })
