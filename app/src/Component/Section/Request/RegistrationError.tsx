    ...
    <>
      {Array.isArray(props.errorMessages) &&
        ...
        props.errorMessages.map((x, index) => {
          return (
            <Form.Item key={index + 1} label={<label style={{ color: "red" }}>{x.Name}</label>} {...layout}>
              {x.IsWaive && <Button onClick={() => props.onWaive(x.Name, x.RequestName)}>Waive</Button>}
              {x.Name === REGISTRATION_VERIFICATION_NAME.REGISTRATION_QUESTION_CHECK && (
                <Button type="link" onClick={() => (props.openModal ? props.openModal(props.jsonData) : null)}>
                  Answer
                </Button>
              )}
              {x.Name === REGISTRATION_VERIFICATION_NAME.SECTION_VALIDITY_CHECK && x.Details !== null && (
                <Typography.Text style={{ color: "red" }}>{x.Details[0]}</Typography.Text>
              )}
              {x.Name !== REGISTRATION_VERIFICATION_NAME.SECTION_VALIDITY_CHECK && x.Details !== null && (
                <Popconfirm title={getDetailsTilte(x.Details)} okText="Ok" cancelText="Cancel">
...
const mapDispatchToProps = (dispatch: Dispatch) => {
  return {
    openModal: (taskJson?: any) => dispatch(showRequestQuestionAnswerModal(true, { taskJson }))
  }
}
